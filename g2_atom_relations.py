"""Enumerate bottom-perm-top expansions of atom composites."""
...
class Atom:
    ...
    @property
    def domain(self) -> Order: return self.rule.domain_order

    @property
    def codomain(self) -> Order: return self.rule.codomain_order
...
class EmbeddedAtom:
    ...
    @property
    def domain(self) -> Order: return self.left + self.atom.domain + self.right

    @property
    def codomain(self) -> Order: return self.left + self.atom.codomain + self.right

    def word(self, flipped: bool = False) -> str:
        return tensor_embed(branching_symbol(self.atom.rule, flipped=flipped), self.left, self.right)
    ...
    def apply(self, tensor: Tensor, inverse: bool = False) -> Tensor | None:
        ...
        if len(tensor) != len(block_order):
            return None
        image = apply_branching_rule(self.atom.rule, tuple(tensor[pos : pos + block_width]), inverse=inverse)
        if image is None:
            return None
...
class AtomComposite:
    ...
    @property
    def family(self) -> str: return "HI1"

    @property
    def domain(self) -> Order: return self.top.codomain

    @property
    def codomain(self) -> Order: return self.bottom.codomain
...
class CrossingComposite:
    ...
    @property
    def family(self) -> str: return "HI2"
    ...
    def crossing_terms(self) -> tuple[str, ...]:
        ...
        for pos in self.positions:
            terms.append(tensor_embed(crossing_symbol(order[pos : pos + 2]), order[:pos], order[pos + 2 :]))
            order = swap_order(order, pos)
        return tuple(terms)

    def word(self) -> str:
        return compose_terms([self.top.word(flipped=True), *self.crossing_terms(), self.bottom.word()])
    ...
    def apply(self, tensor: Tensor) -> Tensor | None:
        ...
        order = self.top.domain
        for pos in self.positions:
            crossed = sage_commutor(order[pos : pos + 2], outputs[pos : pos + 2])
...
class BottomCrossingComposite:
    ...
    @property
    def family(self) -> str: return "HI3"
    ...
    def word(self) -> str:
        return compose_terms(list(self.crossing_terms()) + [self.bottom.word()])
...
class RightCrossingThenAtomComposite:
    ...
    @property
    def family(self) -> str: return "OR1"
    ...
    @property
    def codomain(self) -> Order: return self.atom.codomain

    def crossing_word(self) -> str:
        order = self.domain
        return tensor_embed(crossing_symbol(order[-2:]), order[:-2], ())

    def word(self) -> str:
        return compose_terms([self.crossing_word(), self.atom.word()])

    def apply(self, tensor: Tensor) -> Tensor | None:
        crossed = sage_commutor(self.domain[-2:], tensor[-2:])
...
class OverlappingAtomComposite:
    ...
    @property
    def family(self) -> str: return "OR2"

    @property
    def domain(self) -> Order: return self.atom_b.domain

    @property
    def codomain(self) -> Order: return self.atom_a.codomain
    ...
    def word(self) -> str:
        return compose_terms([self.atom_b.word(), *self.crossing_terms(), self.atom_a.word()])
...
class RotateRightStrandThenAtomComposite:
    ...
    @property
    def family(self) -> str: return "OR3"
    ...
    def word(self) -> str:
        return compose_terms(list(self.crossing_terms) + [self.atom.word()])
...
def atoms() -> tuple[Atom, ...]:
    # The atom list is just the non-identity branching rules.
    atoms = [Atom(rule) for rule in BRANCHING_RULES if rule.old_suffix]
    return tuple(sorted(atoms, key=lambda atom: (atom.domain, atom.codomain)))
...
def atom_composites(atom_list: tuple[Atom, ...]) -> tuple[AtomComposite, ...]:
    # Enumerate HI1 relations: a flipped atom followed directly by an atom.
...
def single_crossing_composites(atom_list: tuple[Atom, ...]) -> tuple[CrossingComposite, ...]:
    ...
    seen = set()
    for top in atom_list:
        for left_label in (1, 2):
            if (left_label, top.domain[0]) not in {(1, 2), (2, 1)}:
                continue
...
def orders_with_block(labels: Order, width: int, block: Order, pos: int) -> tuple[Order, ...]:
    ...
    for filler in product(labels, repeat=free):
        order = filler[:pos] + block + filler[pos:]
        out.append(order)
    return tuple(out)
...
def bottom_crossing_composites(atom_list: tuple[Atom, ...]) -> tuple[BottomCrossingComposite, ...]:
    # Enumerate HI3 relations: precompose an atom with a left-convention permutation moving its last strand.
...
def right_crossing_then_atom_composites(atom_list: tuple[Atom, ...]) -> tuple[RightCrossingThenAtomComposite, ...]:
    # Enumerate OR1 relations: cross the last two strands, then apply an atom with a right identity.
    out: list[RightCrossingThenAtomComposite] = []
    for atom in atom_list:
        for right_label in (1, 2):
...
def common_middle_for_or2(atom_a: Atom, atom_b: Atom, overlap: int) -> tuple[Order, EmbeddedAtom, EmbeddedAtom] | None:
    # Place A at the left and B at the right, requiring their middle words to overlap.
...
def overlapping_atom_composites(atom_list: tuple[Atom, ...]) -> tuple[OverlappingAtomComposite, ...]:
    # Enumerate OR2 relations, including the middle permutation P.
...
def rotate_right_strand_then_atom_composites(atom_list: tuple[Atom, ...]) -> tuple[RotateRightStrandThenAtomComposite, ...]:
    # Enumerate OR3 relations: print the crossing string from the right end back toward A.
    out: list[RotateRightStrandThenAtomComposite] = []
    for atom in atom_list:
        if len(set(atom.domain)) != 1:
            continue
        right_label = 1 if atom.domain[0] == 2 else 2
        embedded = EmbeddedAtom(atom, (right_label,), ())
        order = atom.domain + (right_label,)
        crossing_terms: list[str] = []
        for pos in range(len(atom.domain) - 1, -1, -1):
            crossing_terms.append(tensor_embed(crossing_symbol(order[pos : pos + 2]), order[:pos], order[pos + 2 :]))
            order = order[:pos] + (order[pos + 1], order[pos]) + order[pos + 2 :]
        out.append(RotateRightStrandThenAtomComposite(embedded, tuple(crossing_terms)))
    return tuple(sorted(out, key=lambda composite: (len(composite.domain), composite.domain, composite.codomain)))


def relation_family(atom_list: tuple[Atom, ...], name: str):
...
def selected_family_names(name: str) -> tuple[str, ...]:
    if name == "all":
        return ("HI1", "HI2", "HI3", "OR1", "OR2", "OR3")
    return (name,)


def tensor_weight(order: Order, tensor: Tensor):
    return sage_weight(to_sage(order, tensor))


def highest_rows(domain: Order, codomain: Order) -> tuple[tuple[Tensor, Tensor], ...]:
    ...
    return tuple(
        ...
        if tensor_weight(domain, domain_tensor) == tensor_weight(codomain, codomain_tensor)
    )


def map_vector(domain: Order, codomain: Order, rows: tuple[tuple[Tensor, Tensor], ...], morphism) -> list:
...
def expand_in_bpt(composite):
    # Expand a relation in the bottom-perm-top basis using highest-weight tensors.
    basis = bpt_basis(composite.domain, composite.codomain)
    if not basis:
        return ()
    rows = highest_rows(composite.domain, composite.codomain)
    columns = [map_vector(composite.domain, composite.codomain, rows, diagram.apply) for diagram in basis]
    rhs = vector(QQ, map_vector(composite.domain, composite.codomain, rows, composite.apply))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Enumerate bottom-perm-top expansions of atom composites.")
    parser.add_argument("--family", choices=("HI1", "HI2", "HI3", "OR1", "OR2", "OR3", "all"), default="HI1")
    ...
    for name in selected_family_names(args.family):
        family = relation_family(atom_list, name)
